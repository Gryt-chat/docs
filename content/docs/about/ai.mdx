...
Changes here always go through a pull request, with no direct pushes. I read every line of code before it merges, however small the change. The one exception is a pull request that only touches a README or other documentation, like fixing a link. That can merge once its checks pass without waiting for me, because it can't change what the software does.
