---
title: Inside the client
description: How the desktop and web client is laid out, and how to run it locally
icon: Monitor
---
...
The voice engine is [`@gryt/voice`](/docs/build/voice), an npm package. `packages/webRTC` holds the adapters that wire it into this client: the settings store mapped into a `VoiceConfig`, the socket behind a `RoomCoordinator`, and the Electron bridge behind a `VoiceHost`.
...
See [Configuration](/docs/host/configuration) for the full list.
