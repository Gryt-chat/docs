...
When you join a server, you prove your identity through a [challenge-response protocol](/docs/about/security) — the server never receives a reusable token it could replay. The server you join can still store and process the content you send to it (messages and uploads), because that's how server-hosted communication works.

## Trust boundaries (what you're trusting)

Here's the honest version of the trust model:

- **If you join someone else's server**, you're trusting that server operator with the data their server stores. You are **not** trusting them with your identity — they can't impersonate you.
...
- **Storage**: the database and uploaded files, side by side on the server's disk
...
graph TB
  ...
  server --> db[Database]
  server --> files[Uploaded_files]
  client -->|OIDC_login| auth[Identity_provider]
  client -->|Certificate| identity[Identity_service]
  server -->|JWKS_verify| identity
```

If you want the deeper technical diagrams and data flow, see the full [Architecture](/docs/about/architecture) page.
...
No. Servers verify identity certificates by fetching a public key from the Identity Service's JWKS endpoint. There's no central directory, no approval process, no "nobody knows my server exists" problem.
...
Every commit that had AI assistance keeps its `Co-Authored-By` trailer, and the review-required areas have PR history on top of that, so you can check this yourself rather than take it on trust. The [AI policy](/docs/about/ai) has the exact paths, and is honest about the limits of a one-person review process.

## Further reading

- [Security](/docs/about/security)
- [Architecture](/docs/about/architecture)
- [Quick Start](/docs/host/quick-start)
- [Docker Compose deployment](/docs/host/docker-compose)
- [Configuration](/docs/host/configuration)
- [AI policy](/docs/about/ai)
- [Licensing](/docs/about/licensing)
