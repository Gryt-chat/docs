...
<Callout type="warn">
Getting this wrong fails quietly. You get `Failed to initialize RNNoise processor` in the console, and voice keeps working without noise suppression. Nothing else breaks, so it's easy to ship.
</Callout>

## Set the host

`VoiceHost` answers two questions about the runtime it's embedded in. In a browser you can skip this entirely: the default is `webHost`, which reports no native capture and no insecure transport, so forgetting to set one degrades to the plain `getUserMedia` path rather than throwing.
...
Set it at module scope rather than inside an effect. Host capabilities can't change while the process is running, and setting it during a render leaves the first connection asking a host that isn't there yet.
...
Mount it once, above anything that consumes a voice hook. If your app has its own singleton-hook host, this goes next to it rather than instead of it: they're two separate registries.
...
Config is a context rather than a module singleton on purpose. It changes while a call is running, every time somebody drags a slider, and a singleton wouldn't re-render anything. Host capabilities are fixed for the life of the process, which is why that one is a singleton.
...
The full surface is in [Hooks](/docs/build/voice-hooks).
...
`voiceConfig.ts` maps the client's settings store into a `VoiceConfig`. It's the one place that knows both shapes, so renaming a setting is a change here rather than a change inside the package.
