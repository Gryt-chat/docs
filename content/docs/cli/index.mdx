...
| `enter` | One server, with its addresses in full |
| `n` | New server |
...
The `ADDRESS` column is the address other machines can use. A server bound to
`0.0.0.0` is reachable on every address this machine has, and `enter` lists them
all with loopback last.
