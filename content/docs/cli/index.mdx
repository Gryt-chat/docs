...
A cap is about the machine running the SFU rather than about ports. Every
participant's media shares one UDP port, so there is no per-person port to run
out of. What runs out first is CPU and upload bandwidth.
