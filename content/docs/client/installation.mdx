...
| `Gryt-Chat-x.x.x-win-x64-portable.exe` | Runs without installing, for a USB stick or a machine you cannot install on |
...
The app is code-signed and notarized, so it opens normally. There is no
right-click-Open step, and if macOS does complain that it cannot be checked for
malicious software, that is worth reporting rather than working around.
