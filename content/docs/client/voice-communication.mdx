...
**Mute** stops your microphone. It's the last stage of the audio chain, so
...
while hearing nothing isn't a state anybody wants to be in by accident.
...
desktop app the key works while Gryt is in the background; a browser can't see
keys it doesn't have focus for, so the web client's push-to-talk only works
...
output needs a native helper that excludes Gryt's process tree, so it's a
...
The SFU forwards whatever the two ends negotiate. It doesn't transcode, so the
codec choice is between you and the people watching.

## When the server restarts

Voice keeps working. This is worth knowing because it looks alarming and isn't.
...
nothing about any of that. Restarting the server doesn't touch the media path,
...
3. The client fully disconnects and reconnects to the SFU, rather than trying to
   ...
   in the member list are all derived from that state, and rebuilding it's more
...
voice audibly blips even though step 1 and 2 didn't need it.
...
measured at the SFU. The ring around an avatar is that client saying it's
talking.

That means indicators depend on the signalling connection, and audio doesn't.
...
If you're on the same network as the SFU and the remote address is a private
one at near-zero latency, media is staying on the LAN. A public address at 20 ms
or more means it's going out to the internet and back, which is expected when
the SFU is hosted elsewhere and worth investigating when it isn't.
