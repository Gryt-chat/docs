...
| `GRYT_AUTH_MODE` | `required` | `required` admits people normally. `disabled` rejects every join while the server is out of service. Any other value also rejects joins, and reports itself as misconfigured. |
