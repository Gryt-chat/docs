...
- **One UDP media port** — the SFU needs `ICE_UDP_MUX_PORT` reachable from the internet, and every participant's media shares it. The compose files here set it to 443; a server made with the `gryt` CLI uses 3478.
...
| `443` | UDP | SFU WebRTC media, `ICE_UDP_MUX_PORT`. Must be public |
