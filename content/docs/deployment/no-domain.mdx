...
| `443` | UDP | The actual voice and video, `ICE_UDP_MUX_PORT` |
...
It is one port, not a range, and 443 is what the compose files here set it to.
Restrictive networks usually leave 443 alone, so people behind a corporate or
hotel firewall stand a better chance of getting through.

Change it with `ICE_UDP_MUX_PORT` and forward whatever you pick. Two reasons you
might: 443 needs a privileged bind, and it clashes with anything already serving
HTTPS on that machine. A server made with the `gryt` CLI uses 3478 instead, which
is also what the SFU falls back to when nothing sets the variable.
