...
Voice does not travel over the same connection as the chat, and it is UDP, not
...
<Callout type="warn" title="Not 0.0.0.0">
  `0.0.0.0` means "every interface" when you are deciding what to *listen* on,
  which is why `HOST=0.0.0.0` is correct. These two are different: they are
