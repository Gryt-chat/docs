...
No. The SFU, the authentication and identity code, the client's key handling, the image worker, and the data layer are written by hand. Documentation, the website, most of the interface, and dev tooling may be AI-assisted.

Commits made with AI assistance keep their `Co-Authored-By` trailer, so you can verify this in the git history. See the [AI policy](/docs/guide/ai) for the exact paths and the rules for contributors.
