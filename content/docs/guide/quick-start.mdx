...
1. Download and install the [Gryt desktop app](https://github.com/Gryt-chat/gryt/releases)
...
Connect with the [Gryt desktop app](https://github.com/Gryt-chat/gryt/releases) and enter your
server address. A bare IP and port works: `203.0.113.10:5000`. So does a hostname if you have one.

[app.gryt.chat](https://app.gryt.chat) needs a hostname with a certificate, like
`gryt.example.com`. A browser will not connect to a plain IP, because it refuses to open an
insecure connection from an HTTPS page. If you have no domain, see
[No domain, just an IP](/docs/deployment/no-domain).
...
**LAN party?** Enable **Allow anyone on LAN to join** in **Server settings → Overview** to let anyone on the local network join without an invite code. Remote users still need an invite. See [Multi-Server → LAN open join](/docs/server/multi-server#lan-open-join-skip-invite-codes-on-lan).
...
1. Install [Node.js 22.13+](https://nodejs.org/)
2. Download `gryt-server-windows-x64-v*.zip` from the [releases page](https://github.com/Gryt-chat/server/releases)
3. Extract and open `config.env` — set your `SERVER_NAME`
4. Double-click `start.bat`

Connect with the desktop app or [app.gryt.chat](https://app.gryt.chat) at `localhost:5000`.

<Callout type="info">
See the full [Windows deployment guide](/docs/deployment/windows) for firewall setup, remote access, and troubleshooting.
</Callout>
