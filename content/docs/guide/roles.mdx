...
| | Rank | Roughly |
|---|---|---|
| Owner | 100 | Everything. Can't be edited or handed out. |
...
lack, and a role without it's somebody who is in your server and can't see it.

That's what makes a genuinely read-only tier possible — and it's why the Guest
role isn't empty. A read-only role that can't read isn't read-only.

## Who can start a call

Ringing somebody in a direct message is *start calls*. Answering is *join
voice*, the same permission as walking into a voice channel.

Keeping them apart is what lets you limit who places a call without stopping
anyone picking one up. A role with *join voice* and no *start calls* can be
rung by anybody and can answer; it just has no call button of its own. That's
the shape behind "only paying members start calls". One permission covering
both would leave somebody unable to answer a call placed to them.

Every role that could already call keeps it when you upgrade, so nothing stops
working the day you take the update. Take it off the roles you want to
restrict.
...
- **With an account** — a durable identity, vouched for by a certificate
  authority. Losing a device doesn't lose it.
...
isn't expressible with a single default, which is why there are two.
...
on the server, and it doesn't include them by default. Where it doesn't, the
...
its own is something a patient stranger also has, and it's exactly what
...
holding a higher role stays where they are. It's checked when somebody joins
and after they post, so a role earned while they were away arrives the next time
they turn up.

Built-in roles don't offer this. A server that wants an automatic tier wants a
...
rank. One admin can't kick another; only the owner can act on an admin.

The same rule applies to handing roles out. You can't grant a role at or above
your own rank, and you can't put a permission into a role that you don't hold
yourself — otherwise delegating *manage roles* would be delegating everything.

## A worked example

Say you're running a public support server.
