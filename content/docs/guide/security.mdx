...
## Direct messages are not private from the server

A direct message looks private. It sits away from the channels, it is between two
people, and nothing about it suggests anybody else is reading. The person running the
server can.

Direct messages are stored the same way channel messages are: as text, in the server's
database, on hardware somebody else owns. They are not end-to-end encrypted. Whoever
runs the server can read any conversation on it, and so can anyone who gets hold of a
backup.

The identity model above protects who you are. A server cannot impersonate you
elsewhere, cannot replay your proof, and cannot work out that you are also a member
somewhere else. None of that stops it reading what you typed on it.

So pick servers whose operators you would trust with the contents, the same way you
would with any host that keeps your messages.

End-to-end encryption for direct messages is being designed. It will depend on being
able to prove whose key is whose, which Gryt Auth can do for an account and nobody can
do for a guest identity. So it will not cover every conversation, and one that cannot
be encrypted will say so.
