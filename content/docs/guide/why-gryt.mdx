...
Not the parts that matter. The SFU, the authentication and identity code, the client's key handling, and anything that parses untrusted input are written by hand. Documentation, the website, most of the interface, and dev tooling may be AI-assisted.

Every commit that had AI assistance keeps its `Co-Authored-By` trailer, so you can check this yourself rather than take it on trust. The [AI policy](/docs/guide/ai) has the exact paths.
