...
`DATA_DIR`. That's `/data` inside the container, the
`gryt-prod_gryt-prod-server-data` volume under Compose, and `./data` if you're
running the server directly. It holds the uploads too, so this one directory is
the whole server.
...
└── gryt/                      uploads, thumbnails, avatars and emoji
```
...
Compose adds the project name to the front of a volume's name, so it's
`gryt-prod_gryt-prod-server-data` and not `gryt-prod-server-data`. Give
`docker run -v` the short name and Docker makes a new, empty volume under it,
and the backup comes out empty without an error. `docker volume ls` shows the
real names.

If your stack still runs MinIO, the uploads are in a second volume,
`gryt-prod_gryt-prod-minio-data`, which needs its own copy.
[Moving off MinIO](/docs/host/docker-compose#moving-off-minio) moves them into
`DATA_DIR`.
...
docker run --rm \
  -v gryt-prod_gryt-prod-server-data:/data:ro \
  -v "$PWD:/out" \
...
docker compose down
docker run --rm \
  -v gryt-prod_gryt-prod-server-data:/data \
  -v "$PWD:/in" \
...
[Hosting from the app](/docs/host/from-the-app#where-things-live) has the
paths.
