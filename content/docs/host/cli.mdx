...
Existing servers keep the image they already pulled, and restarting alone
doesn't move them to the new channel's image — see [Which version a server
runs](#which-version-a-server-runs) for how to actually pull a newer one.
...
Restarting doesn't pull the newer image — `docker compose restart` reruns the
container you already have. Pull the image yourself first
(`docker pull ghcr.io/gryt-chat/server:latest`, or `latest-beta` on the beta
channel), then stop and start the server with `x` and `s`. The version is
read off the container rather than asked of the server, so it works on images
built long before the server had any way to report it.
