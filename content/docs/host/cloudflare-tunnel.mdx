...
- **Not OK via Tunnel**:
  - WebRTC media (UDP). You must open and forward `ICE_UDP_MUX_PORT` to the SFU host. It's one port.
...
It also fixes the webhook URL the server hands out. The tunnel talks plain http
to the server, so at `0` a copied webhook URL starts `http://`, and posting to
it gets a 301 to https that most senders won't follow.
...
the number of proxies you really have and you're trusting a header the client
can write. There's more on this in
[Configuration → Running behind a proxy](/docs/host/configuration).
...
## Running cloudflared over HTTP/2

cloudflared defaults to QUIC, which needs outbound UDP to Cloudflare's edge. If UDP is blocked on the host network, or the tunnel keeps dropping overnight, switch it to HTTP/2 instead.
...
Cloudflare Tunnel may occasionally reset long-lived WebSocket connections during edge server rotation or tunnel reconnection. Gryt handles this automatically: the signaling server preserves voice state for up to 15 seconds during transient disconnects. From app version 1.11.34 (voice#65), the client recovers without tearing down the SFU media connection if it's still alive. On older versions the SFU socket doesn't survive the reset. Users may briefly see a "Reconnecting" toast but voice/screenshare should continue uninterrupted on 1.11.34+.
...
- **[Caddy](https://caddyserver.com/)** — simple reverse proxy with automatic HTTPS (see the [Docker Compose guide](/docs/host/docker-compose#tls-with-caddy-recommended)). No default body size limit.
...
- `ICE_UDP_MUX_PORT/udp` (default `3478/udp`). One port, and the tunnel doesn't carry it
