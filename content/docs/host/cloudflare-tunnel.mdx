...
Cloudflare Tunnel may occasionally reset long-lived WebSocket connections during edge server rotation or tunnel reconnection. Gryt handles this automatically: the signaling server preserves voice state for up to 15 seconds during transient disconnects, and the client will recover without tearing down the SFU media connection if it's still alive. Users may briefly see a "Reconnecting" toast but voice/screenshare should continue uninterrupted.
