...
- [Server](/docs/host/server) — the largest set, plus the four settings that live in its database
- [SFU](/docs/host/sfu) — nine variables, most of which you'll never touch
- [CLI](/docs/host/cli) — writes all of this for you, if you would rather not
...
[SFU page](/docs/host/sfu) for what replaced it and why.
...
The same setting decides the scheme of the webhook URL the server hands out.
Behind a tunnel the socket is plain http, so at `0` the URL you copy from
Server settings → Webhooks starts `http://`, and a post to it gets a redirect
most webhook senders won't follow. With hops set, the server reads
`x-forwarded-proto` from the same end of the chain.

One for a single reverse proxy. More only if you genuinely chained them.

## Uploads

Uploads are files under `DATA_DIR`, in a folder named by `S3_BUCKET`. The
compose files and the `gryt` CLI set both of these:
...
The server still defaults to `STORAGE_BACKEND=s3` when it's unset, so set it if
you run the server some other way. The folder's setting is called `S3_BUCKET`
because it used to name a bucket.

Keys look like `uploads/<uuid>.<ext>`, and each file has a `.meta` file beside
it holding its content type. The image worker reads and writes the same folder,
so it has to mount the same `DATA_DIR`.
...
through the [management API](/docs/host/server#the-management-api), which is what the
...
never had the text. Files sent in one are encrypted too, so what lands on your
disk is bytes with no name and no type.
