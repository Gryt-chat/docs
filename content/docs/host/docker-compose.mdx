...
| **MinIO** | `pgsty/minio` | S3-compatible file storage | Yes |
...
MinIO stopped publishing its own images, so Gryt uses Pigsty's community build of the same MinIO source instead.
...
- **One MinIO, one bucket.** Every stored object is keyed by a UUID, so two servers writing to the same
  bucket can't collide. Give them separate buckets if you would rather keep the storage separable.
