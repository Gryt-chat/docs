...
Older copies of this compose file ran MinIO for uploads. If yours does, it
keeps working as it is. Moving copies every upload out of the MinIO
volume into the server's data volume, checks each one, and leaves the MinIO
volume alone, so you can go back to it if something's wrong.
...
<Step>
### Download the new compose file and the migration

In the same folder as your `.env`:

```bash
curl -Lo docker-compose.yml https://raw.githubusercontent.com/Gryt-chat/gryt/main/ops/deploy/compose/prod.yml
curl -Lo migrate-minio.yml https://raw.githubusercontent.com/Gryt-chat/gryt/main/ops/deploy/compose/migrate-minio.yml
```

If you changed anything in your old `docker-compose.yml`, carry it over now.
...
```bash
docker compose -f migrate-minio.yml up --abort-on-container-exit --exit-code-from migrate
docker compose -f migrate-minio.yml down
```

It starts a MinIO of its own on the old volume, read-only, and copies the
bucket into `/data/gryt` in the server's volume. Then it reads every object
back from both sides and compares the SHA-256. The last line tells you how it
went:

```
Done. 9 objects copied, every SHA-256 matches, 9 content types written.
```

It reads `S3_BUCKET` from your `.env`, so a bucket with another name is picked
up. It refuses to run if `/data/gryt` already has files in it. If a run fails
partway, delete that folder and run it again:
...
The `MINIO_*` and `S3_*` lines in `.env` don't do anything any more, and can go.
...
`gryt-beta_gryt-beta-server-data` and `gryt-beta_gryt-beta-minio-data`, and
the bucket was `gryt-beta`. For anything else, `docker volume ls` shows yours.
Pass them in when you run the migration:

```bash
MINIO_VOLUME=gryt-beta_gryt-beta-minio-data DATA_VOLUME=gryt-beta_gryt-beta-server-data \
  S3_BUCKET=gryt-beta docker compose -f migrate-minio.yml up --abort-on-container-exit --exit-code-from migrate
```
