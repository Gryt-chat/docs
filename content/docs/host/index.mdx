---
title: Hosting Gryt
description: Run a server, on anything from a spare PC to a VPS
icon: Server
---
...
<Cards>
  ...
  <Card
    title="Tailscale"
    description="Carries voice as well as chat, because WireGuard is UDP and a tunnel isn't."
    href="/docs/host/tailscale"
  />
  <Card
    title="Backups"
    description="What to copy, and why copying gryt.db on its own can hand you an empty database."
...
|--------|----------|-----|-------------------|
| [Embedded (desktop app)](/docs/host/from-the-app) | LAN parties, quick hosting, zero setup | N/A (local) | No (LAN only) |
| [Docker Compose](/docs/host/docker-compose) | Self-hosting on a VPS or bare metal | Add Caddy/Nginx | Yes (443 TCP, 3478 UDP) |
| [Windows (no Docker)](/docs/host/windows) | LAN parties, local teams, Windows PCs | Manual / reverse proxy | Yes |
| [Pelican panel](/docs/host/pelican) | A Pelican or Pterodactyl panel you already run | Manual / reverse proxy | Yes (3 allocations) |
| [Terminal manager](/docs/host/cli) | A machine you administer, one or several servers | Add Caddy/Nginx | Yes |
| [Cloudflare Tunnel](/docs/host/cloudflare-tunnel) | Hosting behind NAT / no static IP | Automatic | UDP only |
| [Tailscale](/docs/host/tailscale) | A private group, no public exposure at all | N/A (inside the tailnet) | No |
| [No domain, just an IP](/docs/host/no-domain) | Desktop-app users, no certificate | None | Yes |
...
See the full [Docker Compose guide](/docs/host/docker-compose) for configuration, TLS, upgrades, and production hardening.
...
Prometheus + Grafana stack is included — see the [Monitoring guide](/docs/host/monitoring).
