---
title: The Gryt server
description: The Gryt server — chat, identity, permissions and everything that isn't media
icon: Server
---
...
A working server is usually three containers:
...
| Variable | Default | What it does |
|----------|---------|--------------|
| `STORAGE_BACKEND` | `s3` | Set it to `filesystem`. The compose files and the CLI do |
| `S3_BUCKET` | — | The folder under `DATA_DIR` that uploads go in, `gryt` in the compose files |
| `DISABLE_S3` | — | Turn uploads off entirely |
...
Keys are flat, `uploads/<uuid>.<ext>`, under `DATA_DIR/<S3_BUCKET>`. The image
worker has to mount the same `DATA_DIR`, because it writes thumbnails there.
...
These cover invites only. [Rate limiting](/docs/host/rate-limiting) covers every
...
The full REST and Socket.IO surface is in the [API reference](/docs/build/server-api).
