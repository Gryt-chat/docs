...
It's a Go service built on [Pion WebRTC v4](https://github.com/pion/webrtc),
and it's deliberately small: one of it runs per machine, shared by every Gryt
...
layers for a receiver that can't keep up. It doesn't do that on its own.
...
| `SFU_CONTROL_PORT` | `9092` | Where Gryt servers register. Don't publish it or forward it |
| `SFU_CONTROL_HOST` | every address | The address `SFU_CONTROL_PORT` listens on. Set `127.0.0.1` when the SFU and its server run straight on one machine, as in the desktop app or a self-hosted bundle. Has to be an IP |
| `SFU_METRICS_PORT` | `9091` | Prometheus metrics. `0` stops serving them |
| `SFU_METRICS_HOST` | every address | The address `SFU_METRICS_PORT` listens on. Has to be an IP |
...
| `SFU_REQUIRE_CLIENT_TOKEN` | `true` | Refuse joins without a signed token. `false` still lets a client in with the old shared server password |
| `SFU_PING_INTERVAL` | `30` | Seconds between pings on a quiet connection. `0` turns liveness checks off |
| `SFU_PONG_TIMEOUT` | `90` | Seconds a peer can stay silent before it's dropped. Raised to two ping intervals if set lower |
| `SFU_CALL_ALONE_TIMEOUT` | `120` | Seconds one person can be alone in a call before the SFU ends it. `0` turns it off |
...
The default is 3478, the IANA STUN port. It needs no privileged bind, and it's
the UDP port a locked-down network has most likely already opened, since
Microsoft Teams requires outbound 3478–3481.

UDP 443 looks like the better choice and generally isn't. Enterprise firewall
vendors recommend blocking it, because QUIC on UDP 443 can't be TLS-inspected
and blocking it forces browsers back to TCP where it can be. On the networks you
would pick 443 to get through, it's the one most likely to be shut on purpose.

Ports are per-protocol either way. UDP 443 and TCP 443 are different sockets, so
an HTTPS server never collides with media on UDP 443. HTTP/3 does, because that
is UDP 443 as well, and Caddy serves
it by default.

The SFU binds this port at startup and refuses to start if something else holds
it. That's deliberate: the alternative is coming up healthy and handing out
candidates on ports nobody opened.

<Callout type="warn" title="The port range is gone">
`ICE_UDP_PORT_MIN` and `ICE_UDP_PORT_MAX` were removed. If they're still set
anywhere, they're ignored. A range never bought capacity — one muxed port
...
| `GET /metrics` | Prometheus, on `SFU_METRICS_PORT` rather than the main port |
| `GET /` | WebSocket upgrade. Anything else gets 400 and a sentence saying so |
| `WS /server` | The Gryt server's control connection, on `SFU_CONTROL_PORT`. The main port answers 403 and names the control port |
...
There are two kinds of connection, and they're told apart by path. `/server` is
...
without a reply if they don't match.
...
every server there — see the [CLI docs](/docs/host/cli).
