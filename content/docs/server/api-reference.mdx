...
| GET | `/info` | Public server preview: `{ name, description, members, version, lanOpen }` |
...
|--------------------------|-------------|
| `server:settings:update` | Update server settings (owner). Fields: `displayName`, `description`, `avatarMaxBytes`, `uploadMaxBytes`, `emojiMaxBytes`, `profanityMode`, `profanityCensorStyle`, `systemChannelId`, `lanOpen` |
