...
Looking to run a Gryt server? See the deployment guide for your platform:

- [Docker Compose](/docs/deployment/docker-compose) (recommended for Linux/VPS)
- [Windows (no Docker)](/docs/deployment/windows)
- [Cloudflare Tunnel](/docs/deployment/cloudflare-tunnel)
- [Kubernetes with Helm](/docs/deployment/kubernetes)
...
# Identity — who this server lets in. See the Identity page.
# GRYT_IDENTITY_TIERS=account,local   # also accept people with no Gryt account
# GRYT_OIDC_ISSUER=https://auth.gryt.chat/realms/gryt
# GRYT_TRUSTED_CERT_ISSUERS=https://id.gryt.chat

# Maintenance switch. Set false to reject every join without stopping the server.
...
```bash
GRYT_IDENTITY_TIERS=account                        # default
GRYT_OIDC_ISSUER=https://auth.gryt.chat/realms/gryt
GRYT_OIDC_AUDIENCE=gryt-web
```

`GRYT_SERVER_ENABLED` is a separate maintenance switch. Set it to `false` to
reject all joins while leaving the server process running.
...
Messages are **created** via the WebSocket `chat:send` event, not REST.

### Uploads
- `POST /api/uploads` (multipart) — stores file in S3, generates thumbnail if image, returns `{ fileId, key, thumbnailKey }`

See the [API Reference](/docs/server/api-reference) for the full list of endpoints.
...
sequenceDiagram
    ...
    C->>S: voice:room:request(roomId)
    S-->>C: voice:room:granted (join_token, sfu_url)
    C->>SFU: WebSocket connect
    C->>SFU: client_join (join_token)
    SFU-->>C: room_joined
    SFU-->>C: offer (SDP)
    C->>SFU: answer (SDP)
    ...
    Note over C,SFU: Media flows directly via UDP (DTLS-SRTP)
```
