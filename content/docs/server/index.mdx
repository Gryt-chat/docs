...
Zero by default, and that matters: `x-forwarded-for` is a header any client can
set, so trusting it blindly lets one client walk around every per-IP limit by
