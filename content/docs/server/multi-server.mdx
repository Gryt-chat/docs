...
believe. `GRYT_TRUSTED_PROXY_HOPS` is zero by default, because that header is
something any client can set, so trusting it blindly would let somebody claim a
private address and walk straight past the invite requirement.

Left at zero behind a proxy, every client arrives wearing the proxy's address.
If that address is private, which it usually is, LAN open join lets in everybody
who can reach the proxy. Set the hop count to match your setup. See
