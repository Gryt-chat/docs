...
<Callout type="info">
LAN discovery only works in the desktop (Electron) app. Browsers can't perform mDNS queries, so the web app always requires manual entry.
</Callout>
...
- If avahi isn't available, it falls back to the `bonjour-service` npm package
- The Electron client browses for `_gryt._tcp` services and shows what it finds
- On shutdown the server withdraws the advertisement, waiting for the goodbye packets before the process exits so the record doesn't outlive it
...
two servers on one host can't share a port.
