...
Delete a message. That one's coming — it needs some untangling inside the chat
handler first, because deleting a message also cleans up its attachments, its
thread counters and the cache, and none of that can be called from outside yet.

So for now an automod can take the person out and can't take the post down.
