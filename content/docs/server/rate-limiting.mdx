...
each, so an account can't sit and mass-report. And `join:requests` is measured
...
Bans are logged as they're applied, with the key, the score and the ban length:
