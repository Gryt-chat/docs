...
description: Finding out why voice won't connect
icon: Bug
---
...
<Callout type="warn">
Cloudflare Tunnel doesn't carry UDP. If your signalling goes through a tunnel,
...
The `host` candidate says 3478 because that's what the SFU bound. The `srflx`
candidate says 57599 because that's what the outside world actually saw. When
...
- `Join validation failed` — the room isn't registered, or the server
  credentials don't match. That's the Gryt server's side, not the client's.
...
in the latency panel. It only fills in while you're connected to voice:
...
If you're on the same network as the SFU and ICE remote shows a `192.168.x.x`
...
off by default and are separate from the latency panel. There's no third
...
No `srflx` line means STUN isn't getting out from the client either, which is a
...
- `ICE_UDP_MUX_PORT`, whether it's published, and whether it's open in the
  firewall
