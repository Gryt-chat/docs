...
entirely. The gating is done by the mute node instead, since that's exactly what
...
There's a separate monitor tap before the mute node, which is what loopback
...
Capturing system audio without also capturing Gryt's own output isn't something
...
it's server-side, so it follows the installation rather than the account.
...
[voice debugging page](/docs/host/voice-debugging) is the better place.
