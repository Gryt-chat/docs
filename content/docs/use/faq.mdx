...
**Server:** Node.js 22.13+ (npm or yarn) — the SQLite support built into Node arrived in that release, and 22.12 isn't enough. Go 1.21+ for the SFU. Docker 20.10+ for containerized deployment.
...
Yes, for personal and non-commercial use. Gryt is source-available under the [AGPL-3.0 license](/docs/about/licensing). You can self-host it, modify it, and use it however you like — as long as you share any changes you make under the same license. If you want to use Gryt commercially or offer it as a service, [get in touch](mailto:sivert@gryt.chat) about a commercial license.

## Security and privacy

### Can a malicious server owner steal my identity?

No. Gryt uses [challenge-response authentication](/docs/about/security) — your Keycloak token is never sent to community servers. Instead, you prove your identity by signing a challenge with a private key that never leaves your device. Even if a server operator captures everything from the join handshake, they can't impersonate you on other servers.
...
### Is the security-critical code written by AI?

AI may be involved, but it never merges unread. The SFU, the authentication and identity code, the client's key handling, the image worker and the data layer can only change through a pull request that gets reviewed line by line — those paths take no direct commits.

Commits made with AI assistance keep their `Co-Authored-By` trailer, and the review-required paths have public PR history as well, so you can verify both in the git history. See the [AI policy](/docs/about/ai) for the exact paths and the rules for contributors.
...
`GRYT_IDENTITY_TIERS` decides which identities may join. Its default, `account`,
requires a Gryt account. Add `local` to accept people without one.

To take the server out of service without stopping it, set
`GRYT_SERVER_ENABLED=false`. That rejects every join regardless of identity.

It used to be `GRYT_AUTH_MODE=disabled`, which still works. The old name
suggested it controlled whether members needed an account, which it never did:
that is `GRYT_IDENTITY_TIERS`, and the two don't interact.

### Do servers need to be registered anywhere?

No. Any server can verify identity certificates by fetching the Identity Service's public key from its JWKS endpoint. There's no central registry of servers. Run the binary, point it at the JWKS URL, and it works — like TeamSpeak, but with cryptographic identity.
...
Yes. You can enable push-to-talk in **Settings → Hotkeys** and bind it to any key. In Electron, the hotkey works globally (even when Gryt isn't focused). Channels can also require push-to-talk via channel settings.
...
This is expected for open-source apps that aren't signed with an expensive code signing certificate. The app is fully open source — you can inspect every line of code or [build it yourself](/docs/about/build-the-client).

- **Windows**: Click **More info → Run anyway** on the SmartScreen prompt.
- **macOS**: Right-click the app → **Open** → **Open** again.

See the [Code signing and OS trust](/docs/about/build-the-client#code-signing-and-os-trust) section for details.

## Is Gryt down, or is it me?

[status.gryt.chat](https://status.gryt.chat) says which of the hosted parts are
up. It runs somewhere else, so it is still there when they are not. A server
somebody else hosts is not on it — [Troubleshooting](/docs/use/troubleshooting)
covers telling those two apart.

## Where can I get help?

- [Status](https://status.gryt.chat)
