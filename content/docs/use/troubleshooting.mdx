...
## Check whether it is you

[status.gryt.chat](https://status.gryt.chat) reports on the parts of Gryt that
Sivert runs: sign-in, the identity service, the hosted servers and the web
client. It watches them from a machine that is not in the same building, so it
stays up when they do not.

If something there is red, nothing on this page will help — that one is being
worked on. When there is a known problem, signed-in clients also show a banner
across the top with whatever the status page says.

None of this covers a server somebody else hosts. If your own server is
unreachable and the status page is green, the problem is that server or the
network between you and it.

## Installation warnings

### Windows SmartScreen blocks the installer

Windows SmartScreen may show **"Windows protected your PC"** when running the installer. This happens because the app isn't yet signed with an EV code signing certificate. Click **More info → Run anyway**.

If the option doesn't appear, your organization may have a Group Policy blocking unsigned apps. In that case, [build from source](/docs/about/build-the-client) instead.

### macOS says the app can't be checked

Released macOS builds are code-signed and notarized, so this shouldn't happen.
If it does, you're most likely running a build you compiled yourself, or one
that didn't come from the [releases page](https://github.com/Gryt-chat/gryt/releases).
...
- Verify ports 5000 and 5005 aren't blocked by a firewall. Both are TCP, and
  neither carries voice — if chat works and audio doesn't, it's the UDP
...
client checks it hasn't expired. When the server's clock is off, a proof it
...
signed from the clock at the moment it's asked for.

If `timedatectl set-ntp true` answers `NTP not supported`, that's the same
thing: `systemd-timesyncd` isn't installed. Installing chrony above fixes it, or
...
If it doesn't match, something else is answering at that address. Don't unblock it.

To avoid this when moving or rebuilding a server, keep `server-identity-key.json` from
the server's `DATA_DIR` — see [Security](/docs/about/security#authenticating-the-server).

### People connect and chat, but nobody can hear anyone

This is the signature of a wrong or unreachable advertised address, and it's
worth recognising because nothing in the app will say so.

Chat and voice don't travel the same way. The signalling connection uses the
address people joined on, so if they can see the server and send messages, that
address is fine. Media doesn't use it: WebRTC connects directly to whatever the
...
So the fault doesn't look like a network problem. It looks like broken audio.

Check, in this order:

- `ICE_ADVERTISE_IP` is your real public IP, not the machine's LAN address. A
  server behind NAT can't discover this by looking at itself.
- `ICE_UDP_MUX_PORT` is open and forwarded to the SFU host. There's one media
  port and it's UDP, 3478 unless you set `ICE_UDP_MUX_PORT` to something
  else.
- The forward points at the machine running the SFU, which isn't necessarily
  the machine running the server.

Testing this from the server itself proves nothing. Some routers will let a
machine reach its own public IP and some won't, and neither answer tells you
...
- `curl -s localhost:5005/health` — a 503 with `"status":"starting"` means its
  UDP readiness probe hasn't passed yet
...
### Joining voice does nothing for fifteen seconds, then blames your network

You click a voice channel, nothing happens for about fifteen seconds, and then
a message says the network may not allow voice traffic through. Trying again
usually works.

It is usually DNS filtering, not the network.

Gryt has no TURN relay, so a call has to find a direct path. Each side asks a
STUN server what its address looks like from outside. That answer is what the
other side connects to. Without one there is nothing to offer, and the browser
spends about fifteen seconds trying before it gives up.

Ad-blocking resolvers — AdGuard Home, Pi-hole, NextDNS — sometimes carry
`stun.l.google.com` on a tracker list. It is a STUN server, not a tracker, but
it is a Google hostname and the lists are broad. When it is blocked it resolves
to `0.0.0.0` and the request goes nowhere.

Check it from a machine on the same network:

```bash
dig +short stun.l.google.com
```

If that comes back `0.0.0.0`, or with no answer, something is filtering it.
Compare against one that is rarely on a list:

```bash
dig +short stun.cloudflare.com
```

Then allow the hostname on whichever resolver is doing it. In AdGuard Home that
is **Filters → Custom filtering rules**:

```
@@||stun.l.google.com^
```

An allowlist rule beats a blocklist entry, even an `$important` one. AdGuard
caches its answers, so give it a minute before testing again.

A blocked STUN server costs more than the one candidate. It also holds ICE
gathering open. A list with one blocked entry and one working entry still gets
a candidate, but gathering never finishes, and that is where most of the
fifteen seconds go.

The server decides which STUN servers clients use, through `STUN_SERVERS`. The
default spans two providers, so one of them being filtered is not fatal. If
yours has been cut down to a single provider, add another.
...
On the **desktop app** (Windows/macOS), Gryt automatically excludes its own audio from the screen share capture using OS-native APIs. If this isn't working:
...
On the **web client** and **Linux desktop app**, there's no OS-level API to separate Gryt audio from other system audio. Your options:
- Use headphones so Gryt voice audio isn't picked up by the screen capture.
...
- See the [Voice Debugging](/docs/host/voice-debugging) guide
...
restoring their session from the JWT. Ensure you're running the latest server
and client versions.
