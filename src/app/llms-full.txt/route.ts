import { BASE_URL, SITE_SUMMARY, docsLlms } from '@/lib/llms';
...
export async function GET(): Promise<Response> {
  const pages = await Promise.all(source.getPages().map(docsLlms.page));
  ...
  const header = [
    ...
    `Single page: append .md to any docs URL, e.g. ${BASE_URL}/docs/about/ai.md`,
    '',
    '---',
    '',
  ].join('\n');
